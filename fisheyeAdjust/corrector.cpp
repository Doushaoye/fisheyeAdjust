...
Mat corrector::latitudeCorrection2(Mat imgOrg, Point2i center, int radius, distMapMode distMap, double camerFieldAngle, camMode camProjMode)
{
	...
	for (int j = 0; j < imgSize.height; j++)
	{
		...
		for (int i = 0; i < imgSize.width; i++)
		{
			...
			float Ax = -30.0 / 180.0*PI;
			float Ay = 0;
			float m0[] = { 1,0,0,0,cos(Ax),-sin(Ax),0,sin(Ax),cos(Ax) };
			float m1[] = { cos(Ay),0,-sin(Ay),0,1,0,sin(Ay),0,cos(Ay) };
			Mat Rx(3, 3, CV_32F,m0);
			Mat Ry(3, 3, CV_32F, m1);
			tmpPt.x /= normPt;
			tmpPt.y /= normPt;
			tmpPt.z /= normPt;

			float Ptf[] = { tmpPt.x,tmpPt.y,tmpPt.z };//最后一个参数用来修改成像面的焦距
			Mat M_Pt(3,1, CV_32F, Ptf);
			Mat result_Pt =  Rx*Ry*M_Pt;
			//cout << result_Pt << endl;
			switch (distMap)
			{
			case PERSPECTIVE:
				x = result_Pt.at<float>(0, 0);
				y = result_Pt.at<float>(1, 0);
				z = result_Pt.at<float>(2, 0);
				break;
			case LATITUDE_LONGTITUDE:

				/*latitude = latitude_offset + atan((double)j/(double)(imgSize.height));
				longitude = longitude_offset + i*dx;
				*/
				latitude = latitude_offset + j * dy;  //*((float)j / ((float)imgSize.height))
				longitude = longitude_offset + i * dx;
				//Convert from latitude cooradinate to the sphere cooradinate
				x = -sin(latitude)*cos(longitude);
				y = cos(latitude);
